...
//The RX buffer
extern volatile u08 I2cReceiveData[I2C_RECEIVE_DATA_BUFFER_SIZE];
extern volatile u08 I2cReceiveDataIndex;
extern volatile u08 I2cReceiveDataLength;
...
//! Set the user function which is called when a transmission stops (for example to put a queued transmission into the buffer)
// The stop handler MUST:
// - Manually transmit a stop condition if there is nothing left to send
// - Manually set I2C_State to idle if there is nothing left to send (or BEFORE calling MassterRx/Tx)
...
//Starts a reception into the buffer
